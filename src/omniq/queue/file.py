"""File queue implementations for OmniQ.

This module provides concrete file-based implementations of the queue interface:
- AsyncFileQueue and FileQueue: Task queue with locking mechanism using fsspec
...
class AsyncFileQueue(BaseQueue):
    """Async file-based task queue implementation.
    
    Features:
    - Multiple named queues support using fsspec
    - Transactional task locking with file-based locks
    ...
    async def _acquire_lock(self, queue_name: str, task_id: uuid.UUID, lock_timeout: Optional[timedelta] = None) -> bool:
            ...
            for lock_file in self.fs.glob(f"{queue_path}/*.lock"):
                try:
                    ...
                    if lock_timeout_seconds:
                        expired_time = locked_at + timedelta(seconds=lock_timeout_seconds)
                        if datetime.utcnow() > expired_time:
                            self.fs.rm(lock_file)
                            # Also release asyncio lock if it exists
                            task_id_str = Path(lock_file).stem
    ...
    async def dequeue_async(self, queue_name: str = "default", lock_timeout: Optional[timedelta] = None) -> Optional[Task]:
        ...
        try:
            ...
            task_files = [f for f in self.fs.glob(f"{queue_path}/*.json") 
                ...
                try:
                    ...
                    if await self._acquire_lock(queue_name, task_id, lock_timeout):
                        ...
                        return Task(
                            ...
                            ttl=timedelta(seconds=task_data["ttl_seconds"]) if task_data["ttl_seconds"] else None
                        )
                except Exception:
                ...
                if not queue_file.endswith(".lock"):
                    try:
                        ...
                        if uuid.UUID(task_data["id"]) == task_id:
                            ...
                            await self._release_lock(Path(queue_file).parent.name, task_id)
                            break
                    except Exception:
                        continue
        except Exception:
    ...
    async def list_queues_async(self) -> List[str]:
        ...
        try:
            ...
            return [Path(q).name for q in queue_dirs if self.fs.info(q)["type"] == "directory"]
        except Exception:
            return []


class FileQueue(AsyncFileQueue):
    """Synchronous wrapper for AsyncFileQueue."""
