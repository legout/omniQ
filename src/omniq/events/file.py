...
class AsyncFileEventStorage(BaseEventStorage):
    ...
    async def log_event_async(self, event: TaskEvent) -> None:
        ...
        with self.fs.open(event_path, "w") as f:
            msgspec.json.dump(event_data, f)
            ...
            for event_file in event_files:
                try:
                    with self.fs.open(event_file, "r") as f:
                        event_data = msgspec.json.load(f)
    ...
    async def get_events_by_type_async(self, event_type: str, limit: Optional[int] = None) -> AsyncIterator[TaskEvent]:
        ...
        try:
            ...
            task_dirs = [d for d in self.fs.ls("events", detail=False) 
            ...
            for task_dir in task_dirs:
                ...
                try:
                    ...
                    event_files.sort(key=lambda f: self.fs.info(f)["created"], reverse=True)
