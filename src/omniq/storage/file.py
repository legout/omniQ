# src/omniq/storage/file.py
"""File storage implementations for OmniQ."""
...
class FileResultStorage(BaseResultStorage):
    """Result storage implementation using fsspec for file storage."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize a file result storage.
    ...
    def _initialize_storage(self):
        ...
        results_path = os.path.join(self.base_dir, self.project_name, "results")
        self.fs.makedirs(results_path, exist_ok=True)

    def _get_result_path(self, task_id: str) -> str:
        """Get the path for a result."""
        return os.path.join(self.base_dir, self.project_name, "results", f"{task_id}.result")

    def _get_metadata_path(self, task_id: str) -> str:
        """Get the path for result metadata."""
        return os.path.join(self.base_dir, self.project_name, "results", f"{task_id}.meta")
    ...
    def store(
        ...
        task_result = TaskResult.create(
            task_id=task_id, result=result, status=status, error=error, ttl=ttl
        )
...
class FileEventStorage(BaseEventStorage):
    """Event storage implementation using fsspec for file storage."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize a file event storage.
    ...
    def _initialize_storage(self):
        ...
        events_path = os.path.join(self.base_dir, self.project_name, "events")
