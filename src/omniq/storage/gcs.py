"""
Google Cloud Storage implementation for OmniQ using fsspec and gcsfs.

This module provides GCS-based implementations for task queue,
result storage, event storage, and schedule storage using fsspec
with GCSFileSystem for cloud-based operations.
"""
...
class AsyncGCSQueue(BaseTaskQueue):
    """
    Async GCS-based task queue implementation using fsspec and gcsfs.
    
    This class implements the BaseTaskQueue interface using fsspec
    with GCSFileSystem for persistent task storage with support for
    multiple queues organized in GCS bucket structure.
    """
    ...
    def __init__(
        ...
        gcs_additional_kwargs: Optional[Dict[str, Any]] = None,
        **kwargs
    ):
        """
        Initialize the GCS-based task queue.
        
        Args:
            bucket_name: GCS bucket name for storing task files
            base_prefix: Base prefix/path within the bucket
            ...
            gcs_additional_kwargs: Additional kwargs for gcsfs
        """
        self.bucket_name = bucket_name
        self.base_prefix = base_prefix.strip("/")
        
        # GCS configuration
        self.gcs_kwargs = gcs_additional_kwargs or {}
        if project:
            self.gcs_kwargs["project"] = project
        if token:
            self.gcs_kwargs["token"] = token
        self.gcs_kwargs["access"] = access
        self.gcs_kwargs["consistency"] = consistency
        self.gcs_kwargs["cache_timeout"] = cache_timeout
    ...
    async def connect(self) -> None:
        ...
        try:
            # Import gcsfs here to provide better error messages
            import gcsfs
        except ImportError:
            raise ImportError(
                "gcsfs is required for GCS storage. Install it with: pip install gcsfs"
            )
        
        # Create GCS filesystem instance
        self._fs = fsspec.filesystem("gcs", **self.gcs_kwargs)
        
        # Ensure bucket exists (if we have permissions)
        try:
            if self._fs and not self._fs.exists(self.bucket_name):
                self._fs.makedirs(self.bucket_name, exist_ok=True)
        except Exception:
            # Bucket might exist but we don't have list permissions
            pass
    ...
    def _get_task_path(self, task_id: str) -> str:
        """Get the GCS path for a task."""
        return f"{self.bucket_name}/{self.tasks_prefix}/{task_id}.json"
    
    def _get_queue_path(self, queue_name: str) -> str:
        """Get the GCS prefix for a queue."""
        return f"{self.bucket_name}/{self.queues_prefix}/{queue_name}"
    
    def _get_queue_task_path(self, queue_name: str, task_id: str) -> str:
        """Get the GCS path for a task in a queue."""
        return f"{self.bucket_name}/{self.queues_prefix}/{queue_name}/{task_id}.json"
    
    async def _write_task_file(self, task: Task) -> None:
        """Write task data to GCS."""
        if self._fs is None:
            raise RuntimeError("GCS filesystem not connected")
    ...
    async def _create_queue_link(self, task: Task) -> None:
        ...
        # For GCS, we create a reference file that points to the main task
        reference_data = {
            "task_id": task.id,
            "task_path": self._get_task_path(task.id),
            "priority": task.priority,
    ...
    async def list_tasks(
        ...
        if queue_name:
            ...
            # List all tasks
            tasks_prefix = f"{self.bucket_name}/{self.tasks_prefix}"
            try:
...
class GCSQueue(AsyncGCSQueue):
    """
    Synchronous wrapper around AsyncGCSQueue.
    
    This class provides a synchronous interface to the async GCS queue
...
class AsyncGCSResultStorage(BaseResultStorage):
    """
    Async GCS-based result storage implementation using fsspec and gcsfs.
    
    This class implements the BaseResultStorage interface using fsspec
    for persistent result storage in GCS.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the GCS-based result storage.
        
        Args:
            bucket_name: GCS bucket name for storing result files
            base_prefix: Base prefix/path within the bucket
            project: GCP project ID (optional, can use environment/service account)
            token: Path to service account JSON file or token string
            access: Access mode ('read_only', 'read_write', 'full_control')
    ...
    def _get_result_path(self, task_id: str) -> str:
        """Get the GCS path for a result."""
        return f"{self.bucket_name}/{self.results_prefix}/{task_id}.json"
    ...
    async def list_results(
        ...
        results = []
        results_prefix = f"{self.bucket_name}/{self.results_prefix}"
...
class GCSResultStorage(AsyncGCSResultStorage):
    """
    Synchronous wrapper around AsyncGCSResultStorage.
    
    This class provides a synchronous interface to the async GCS result storage
    implementation using asyncio for execution.
    """
...
class AsyncGCSEventStorage(BaseEventStorage):
    """
    Async GCS-based event storage implementation using fsspec and gcsfs.
    
    This class implements the BaseEventStorage interface using fsspec
    for persistent event logging as JSON files in GCS.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the GCS-based event storage.
        
        Args:
            bucket_name: GCS bucket name for storing event files
            base_prefix: Base prefix/path within the bucket
            project: GCP project ID (optional, can use environment/service account)
            token: Path to service account JSON file or token string
            access: Access mode ('read_only', 'read_write', 'full_control')
    ...
    def _get_event_path(self, event_id: str) -> str:
        """Get the GCS path for an event."""
        return f"{self.bucket_name}/{self.events_prefix}/{event_id}.json"
    ...
    async def get_events(
        ...
        events = []
        events_prefix = f"{self.bucket_name}/{self.events_prefix}"
...
class GCSEventStorage(AsyncGCSEventStorage):
    """
    Synchronous wrapper around AsyncGCSEventStorage.
    
    This class provides a synchronous interface to the async GCS event storage
    implementation using asyncio for execution.
    """
...
class AsyncGCSScheduleStorage(BaseScheduleStorage):
    """
    Async GCS-based schedule storage implementation using fsspec and gcsfs.
    
    This class implements the BaseScheduleStorage interface using fsspec
    for persistent schedule storage and management in GCS.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the GCS-based schedule storage.
        
        Args:
            bucket_name: GCS bucket name for storing schedule files
            base_prefix: Base prefix/path within the bucket
            project: GCP project ID (optional, can use environment/service account)
            token: Path to service account JSON file or token string
            access: Access mode ('read_only', 'read_write', 'full_control')
    ...
    def _get_schedule_path(self, schedule_id: str) -> str:
        """Get the GCS path for a schedule."""
        return f"{self.bucket_name}/{self.schedules_prefix}/{schedule_id}.json"
    ...
    async def list_schedules(
        ...
        schedules = []
        schedules_prefix = f"{self.bucket_name}/{self.schedules_prefix}"
...
class GCSScheduleStorage(AsyncGCSScheduleStorage):
    """
    Synchronous wrapper around AsyncGCSScheduleStorage.
    
    This class provides a synchronous interface to the async GCS schedule storage
    implementation using asyncio for execution.
    """
