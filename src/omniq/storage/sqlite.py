"""
SQLite storage implementation for OmniQ.

This module provides SQLite-based implementations for task queue,
result storage, event storage, and schedule storage.
"""
...
class AsyncSQLiteQueue(BaseTaskQueue):
    """
    Async SQLite-based task queue implementation.
    
    This class implements the BaseTaskQueue interface using SQLite
    ...
    async def connect(self) -> None:
        ...
        await self._db.execute("""
            ...
            ON tasks(queue_name, priority DESC, created_at ASC)
        """)
        await self._db.execute("""
    ...
    async def dequeue(self, queues: List[str], timeout: Optional[float] = None) -> Optional[Task]:
        ...
        queue_placeholders = ",".join("?" * len(queues))
        query = f"""
            SELECT * FROM tasks 
    ...
    async def update_task(self, task: Task) -> None:
        ...
        await self._db.execute("""
            UPDATE tasks SET
                queue_name = ?, priority = ?, status = ?, func = ?,
                args = ?, kwargs = ?, run_at = ?, expires_at = ?,
                retry_count = ?, max_retries = ?, retry_delay = ?,
                dependencies = ?, callbacks = ?, metadata = ?, task_data = ?
            WHERE id = ?
        """, (
            task_data["queue_name"], task_data["priority"], task_data["status"],
    ...
    async def get_queue_size(self, queue_name: str) -> int:
        ...
        cursor = await self._db.execute(
            "SELECT COUNT(*) FROM tasks WHERE queue_name = ? AND status = 'pending'",
            (queue_name,)
        )
        row = await cursor.fetchone()
...
class SQLiteQueue(AsyncSQLiteQueue):
    """
    Synchronous wrapper around AsyncSQLiteQueue.
    
    This class provides a synchronous interface to the async SQLite queue
    implementation using anyio for thread-safe execution.
    """
    
    def connect_sync(self) -> None:
        """Connect to the SQLite database (sync)."""
        asyncio.run(self.connect())
    
    def disconnect_sync(self) -> None:
        """Disconnect from the SQLite database (sync)."""
...
class AsyncSQLiteResultStorage(BaseResultStorage):
    ...
    This class implements the BaseResultStorage interface using SQLite
...
class SQLiteResultStorage(AsyncSQLiteResultStorage):
    """
    Synchronous wrapper around AsyncSQLiteResultStorage.
    
    This class provides a synchronous interface to the async SQLite result storage
    implementation using anyio for thread-safe execution.
    """
...
class AsyncSQLiteEventStorage(BaseEventStorage):
    ...
    This class implements the BaseEventStorage interface using SQLite
    ...
    async def connect(self) -> None:
        ...
        """)
        await self._db.execute("""
            CREATE INDEX IF NOT EXISTS idx_events_worker_id
            ON events(worker_id)
        """)
...
class SQLiteEventStorage(AsyncSQLiteEventStorage):
    """
    Synchronous wrapper around AsyncSQLiteEventStorage.
    
    This class provides a synchronous interface to the async SQLite event storage
    implementation using anyio for thread-safe execution.
    """
...
class AsyncSQLiteScheduleStorage(BaseScheduleStorage):
    """
    Async SQLite-based schedule storage implementation.
    
    This class implements the BaseScheduleStorage interface using SQLite
    ...
    async def connect(self) -> None:
        ...
        """)
        await self._db.execute("""
            CREATE INDEX IF NOT EXISTS idx_schedules_expires_at
            ON schedules(expires_at)
        """)
        await self._db.execute("""
            CREATE INDEX IF NOT EXISTS idx_schedules_ready
            ON schedules(status, next_run)
        """)
    ...
    async def update_schedule(self, schedule: Schedule) -> None:
        ...
        await self._db.execute("""
            UPDATE schedules SET
                schedule_type = ?, func = ?, args = ?, kwargs = ?, queue_name = ?,
                cron_expression = ?, interval_seconds = ?, timestamp = ?, status = ?,
                last_run = ?, next_run = ?, max_runs = ?, run_count = ?,
                ttl_seconds = ?, expires_at = ?, metadata = ?, schedule_data = ?
            WHERE id = ?
        """, (
            schedule_data["schedule_type"], schedule_data["func"], schedule_data["args"],
            ...
            schedule_data["last_run"], schedule_data["next_run"], schedule_data["max_runs"],
            schedule_data["run_count"], schedule_data["ttl_seconds"], schedule_data["expires_at"],
            schedule_data["metadata"], schedule_data["schedule_data"], schedule.id
        ))
    ...
    async def cleanup_expired_schedules(self) -> int:
        ...
        now = datetime.utcnow().isoformat()
        cursor = await self._db.execute(
            "DELETE FROM schedules WHERE expires_at IS NOT NULL AND expires_at <= ?",
            (now,)
        )
...
class SQLiteScheduleStorage(AsyncSQLiteScheduleStorage):
    """
    Synchronous wrapper around AsyncSQLiteScheduleStorage.
    
    This class provides a synchronous interface to the async SQLite schedule storage
    implementation using anyio for thread-safe execution.
    """
