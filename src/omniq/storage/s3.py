"""
S3 storage implementation for OmniQ using fsspec and s3fs.

This module provides S3-based implementations for task queue,
result storage, event storage, and schedule storage using fsspec
with S3FileSystem for cloud-based operations.
"""
...
class AsyncS3Queue(BaseTaskQueue):
    """
    Async S3-based task queue implementation using fsspec and s3fs.
    
    This class implements the BaseTaskQueue interface using fsspec
    with S3FileSystem for persistent task storage with support for
    multiple queues organized in S3 bucket structure.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the S3-based task queue.
        
        Args:
            bucket_name: S3 bucket name for storing task files
            ...
            s3_additional_kwargs: Additional kwargs for s3fs
        """
        self.bucket_name = bucket_name
        self.base_prefix = base_prefix.strip("/")
        
        # S3 configuration
        self.s3_kwargs = s3_additional_kwargs or {}
        if aws_access_key_id:
            self.s3_kwargs["key"] = aws_access_key_id
        if aws_secret_access_key:
            self.s3_kwargs["secret"] = aws_secret_access_key
        if aws_session_token:
            self.s3_kwargs["token"] = aws_session_token
        if region_name:
            self.s3_kwargs["client_kwargs"] = self.s3_kwargs.get("client_kwargs", {})
            self.s3_kwargs["client_kwargs"]["region_name"] = region_name
        if endpoint_url:
            self.s3_kwargs["client_kwargs"] = self.s3_kwargs.get("client_kwargs", {})
            self.s3_kwargs["client_kwargs"]["endpoint_url"] = endpoint_url
    ...
    async def connect(self) -> None:
        ...
        try:
            # Import s3fs here to provide better error messages
            import s3fs
        except ImportError:
            raise ImportError(
                "s3fs is required for S3 storage. Install it with: pip install s3fs"
            )
        
        # Create S3 filesystem instance
        self._fs = fsspec.filesystem("s3", **self.s3_kwargs)
    ...
    async def _create_queue_link(self, task: Task) -> None:
        ...
        # For S3, we create a reference file that points to the main task
        reference_data = {
            "task_id": task.id,
            "task_path": self._get_task_path(task.id),
            "priority": task.priority,
...
class S3Queue(AsyncS3Queue):
    """
    Synchronous wrapper around AsyncS3Queue.
    
    This class provides a synchronous interface to the async S3 queue
...
class AsyncS3ResultStorage(BaseResultStorage):
    """
    Async S3-based result storage implementation using fsspec and s3fs.
    
    This class implements the BaseResultStorage interface using fsspec
    for persistent result storage in S3.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the S3-based result storage.
        
        Args:
            bucket_name: S3 bucket name for storing result files
            base_prefix: Base prefix/path within the bucket
            aws_access_key_id: AWS access key ID
            aws_secret_access_key: AWS secret access key
            aws_session_token: AWS session token (for temporary credentials)
...
class S3ResultStorage(AsyncS3ResultStorage):
    """
    Synchronous wrapper around AsyncS3ResultStorage.
    
    This class provides a synchronous interface to the async S3 result storage
    implementation using asyncio for execution.
    """
...
class AsyncS3EventStorage(BaseEventStorage):
    """
    Async S3-based event storage implementation using fsspec and s3fs.
    
    This class implements the BaseEventStorage interface using fsspec
    for persistent event logging as JSON files in S3.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the S3-based event storage.
        
        Args:
            bucket_name: S3 bucket name for storing event files
            base_prefix: Base prefix/path within the bucket
            aws_access_key_id: AWS access key ID
            aws_secret_access_key: AWS secret access key
            aws_session_token: AWS session token (for temporary credentials)
...
class S3EventStorage(AsyncS3EventStorage):
    """
    Synchronous wrapper around AsyncS3EventStorage.
    
    This class provides a synchronous interface to the async S3 event storage
    implementation using asyncio for execution.
    """
...
class AsyncS3ScheduleStorage(BaseScheduleStorage):
    """
    Async S3-based schedule storage implementation using fsspec and s3fs.
    
    This class implements the BaseScheduleStorage interface using fsspec
    for persistent schedule storage and management in S3.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the S3-based schedule storage.
        
        Args:
            bucket_name: S3 bucket name for storing schedule files
            base_prefix: Base prefix/path within the bucket
            aws_access_key_id: AWS access key ID
            aws_secret_access_key: AWS secret access key
            aws_session_token: AWS session token (for temporary credentials)
    ...
    def _get_schedule_path(self, schedule_id: str) -> str:
        """Get the S3 path for a schedule."""
...
class S3ScheduleStorage(AsyncS3ScheduleStorage):
    """
    Synchronous wrapper around AsyncS3ScheduleStorage.
    
    This class provides a synchronous interface to the async S3 schedule storage
    implementation using asyncio for execution.
    """
