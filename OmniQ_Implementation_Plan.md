...
- **Implementation Pattern**:
  ...
  class TaskQueue:
      ...
      def enqueue(self, task):
          return asyncio.run(self._async_queue.enqueue(task))
      
      def __enter__(self):
          asyncio.run(self._async_queue.__aenter__())
          return self
  ```
...
- **Naming Convention**: Use `Async` prefix for core async classes, no prefix for sync wrappers
...
class TaskQueue:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        asyncio.run(self._async_queue.__aexit__(exc_type, exc_val, exc_tb))
```
...
1. **Async First, Sync Wrapped** approach for maximum performance and flexibility
2. Replacing `obstore` with `fsspec` for more stable and widely-used storage abstraction
3. Adding support for both synchronous and asynchronous interfaces throughout the library
4. Implementing proper context managers for both sync and async usage
5. Adding optional cloud storage support through s3fs, gcsfs, and adlfs
6. Supporting task TTL and automatic cleanup across all storage backends
7. Implementing schedule pause/resume functionality
8. Ensuring proper resource management in both sync and async contexts